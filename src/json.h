...
namespace json {
	...
	public:
		...
		static std::string ToString(JSON object)
		{
		...
		typename enable_if<is_integral<T>::value && !is_same<T, bool>::value, JSON&>::type operator=(T i) {
			SetType(Class::Integral); Internal.Int = (long)i; return *this;
		}
	...
	namespace {
		...
		void consume_ws(const string& str, size_t& offset) noexcept {
			while (isspace(str[offset])) ++offset;
		}

		JSON parse_object(const string& str, size_t& offset) noexcept {
		...
		JSON parse_array(const string& str, size_t& offset) noexcept {
