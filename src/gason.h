...
union JsonValue {
    ...
    inline JsonNode* at(const char* key) const;
};
...
inline bool JsonValue::hasKey(const char* key) const
{
    auto tag = getTag();
    if (tag != JSON_OBJECT || key == nullptr)
        return false;

    JsonNode* n = toNode();
    for(auto nn = n; nn != nullptr; nn = nn->next)
        if (!strcmp(key, nn->key))
            return true;
...
inline JsonNode* JsonValue::at(const char* key) const
{
    auto tag = getTag();
    if (tag != JSON_OBJECT || key == nullptr)
        return nullptr;
